...
btnCalcular.addEventListener("click", async () => {
  ...
  if (respuesta && respuesta.serie) {
    //Si no hay error seguimos con el procedimiento normal.
    ...
    document.querySelector(
      "#resultado"
    ).innerHTML = `Resultado: $ ${cambio.toFixed(2)}`; //Agregar sólo hasta 2 decimales.
  } else {
    //Si hay error mostramos el mensaje.
    errorSpan.innerHTML =
      "No se pudo obtener la información. Intenta nuevamente."; //Mensaje de error en el span.
  }
});
